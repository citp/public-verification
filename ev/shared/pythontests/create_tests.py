...
def pstring(n):
    return str(hex(n))[2:].rjust(32, "0").upper()
def pprint(n, pref=""):
    print(pref + pstring(n))

with open("bench_tests.csv", "w") as csvfile:
    ...
    for _ in range(100):
        ...
        csvwriter.writerow([pstring(ika) + pstring(adkey) + pstring(ikb) + pstring(ek) + pstring(nonce), pstring(out)])
